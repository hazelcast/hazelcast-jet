...
public class CsvQueryTargetDescriptorTest {
    ...
    public void test_create(boolean key) {
        ...
        CsvQueryTargetDescriptor descriptor = new CsvQueryTargetDescriptor(ImmutableMap.of("string", 0));

        // when
        QueryTarget target = descriptor.create(SERIALIZATION_SERVICE, extractors, key);

        // then
        assertThat(target).isInstanceOf(CsvQueryTarget.class);
    }

    @Test
    public void test_serialization() {
        CsvQueryTargetDescriptor original = new CsvQueryTargetDescriptor(ImmutableMap.of("string", 0));

        // when
        CsvQueryTargetDescriptor serialized =
