...
by the <<appendix, backwards compatibility guarantees>>.

== Test Sources

Jet provides some {jet-javadoc}/pipeline/test/TestSources[test and development sources]
which can be used both for batch and streaming jobs.

=== Batch

Jet provides test batch sources which emits the supplied items and
then completes:
...
rate of events per second. It also generated a sequence number and
timestamp with each event which can be used to create mock data:
...
Jet also provides {jet-javadoc}/pipeline/test/Assertions[assertions]
which can be used for testing the intermediate or final output of a
...
point finish emitting items. Instead, we can rely on some timeouts
...
Different to the batch example above, this assertion will be run
...
will fail with an `AssertionError` instead.