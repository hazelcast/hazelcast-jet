...
You can also implement simple aggregation logic with stateful mapping,
like keeping a rolling sum of a value, or its extreme values. You can
also achieve these with <<rolling-aggregation, rolling aggregation>> and
reuse Jet's ready-made aggregate operations.
