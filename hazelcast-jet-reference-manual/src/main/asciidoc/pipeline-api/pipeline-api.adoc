...
{jet-javadoc}/Observer.html[Observers] on them, which are basically call-backs for
...
`Observables` are backed by distributed data structures which should be cleaned-up
by the client, once they are no longer necessary. They have a
{jet-javadoc}/Observable.html#destroy--[destroy method] which does just that. These same
backing data structures will also be cleaned-up automatically after their owner `Jobs`
have completed, failed or have been cancelled. The automatic clean-up is happens by default
7 days after `Job` completion and can be configured via the `jet.job.results.ttl.seconds`
property (see <<system-properties, System Properties>> section).
