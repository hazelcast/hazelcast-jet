...
[[connector-files-hadoop]]
== Hadoop

The File Source and Sink has limited production use due to text only
file format support. For various file formats and file-systems
<<hadoop, Hadoop>> module can be used. Some common file formats for
Apache Hadoop:
...
You can use Hazelcast Jet Hadoop module to read/write files from/to
local file system or any file system Apache Hadoop supports. Various
cloud storages can be configured as a file system for Apache Hadoop
thus enables read/write files(objects). Cloud Storages Apache Hadoop
supports:
