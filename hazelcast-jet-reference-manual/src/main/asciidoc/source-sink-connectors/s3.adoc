...
The connectors expect the user to provide either an
https://github.com/aws/aws-sdk-java/blob/master/aws-java-sdk-s3/src/main/java/com/amazonaws/services/s3/AmazonS3.java[AmazonS3]
client or credentials (Access key ID, Secret Access Key) to create the
...
in bucket for each batch of items. Name of the file will be processor's
global index followed by an incremented counter, for example the
processor having the index `2` will create these consecutive objects
`2-0, 2-1, 2-2...`.
