...
There are two types of "handlers" that can be used for user metrics (for details see the
`{jet-javadoc}/core/metrics/UserMetrics.html[UserMetrics Javadoc]`):

* `{jet-javadoc}/core/metrics/Counter.html[counters]`: useful for incrementally
counting things, like number of certain events
* `{jet-javadoc}/core/metrics/Gauge.html[gages]`: useful for storing measured values,
like queue length, load-factor and such
