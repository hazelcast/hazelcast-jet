...
possible to specify the desired `{jet-javadoc}/core/metrics/Unit.html[measurement unit]`
of the metrics. It's important to note though that this measurement unit
serves a purely informative purpose only (it's being used to set one of the
tags of the metrics, for details see the
`{jet-javadoc}/core/metrics/Unit.html[javadoc]`), no validation is being
done on it. The `Metric` instance for a certain name can be requested multiple times
from the utility class with different `Unit` values and this will not generate
any errors. The first `Unit` value from the first request will take effect,
