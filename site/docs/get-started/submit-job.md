...
```bash
docker run -it -v <path_to_JAR_file>:/jars hazelcast/hazelcast-jet jet -t jet@172.17.0.2 submit -c <main_class_name> /jars/<name_of_the_JAR_file>
```
...
```bash
$ docker run -it hazelcast/hazelcast-jet jet -t jet@172.17.0.2 list-jobs
ID                  STATUS             SUBMISSION TIME         NAME
