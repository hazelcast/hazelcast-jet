...
|hazelcast.lite-member.enabled|false| Specifies whether the IMDG instance behind the Jet is data member or lite member. In our case where IMDG is used as Jet data storage, we recommend that you do not change the default value, as it causes complications rather than a specific advantage of making IMDG a lite member, the default value is false, i.e., data member.
