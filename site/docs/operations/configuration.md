...
|hazelcast.lite-member.enabled|false|Specifies whether the Jet instance acts as a data member or a lite member at the IMDG level. Since Jet uses IMDG services for data storage, we recommend that you do not change the default value, as it increases the latency, the default value is false, i.e., data member. The reason for this latency increase is that accessing data in IMDG partitions has to contain network calls in the lite member. Besides, Jet needs at least one data member in the cluster to run jobs because job coordination primitives are kept in IMDG data structures.|
