...
The most important changes we have made and which have affected Jet too
are as follows:

* Many packages have been renamed and classes have been moved from one
package to another. For details see
...
moved from the `com.hazelcast.jet.function` package to the
...
* Generic parameters on several methods and classes have been
updated/fixed (i.e. `EntryProcessor`) (see
link:https://docs.hazelcast.org/docs/latest/manual/html-single/#introducing-lambda-friendly-interfaces[relevant section]
in IMDG Migration Guide).

* "Group name" config option is now called "cluster name". See next code
snippet. We have applied these changes to the Jet Command Line too
...
* Async methods return `CompletionStage` now rather than the old
...
=== Jet API Changes
We have made multiple breaking changes in Jet's own APIs too:
...
`ServiceFactory`. These properties are relevant in the context of
asynchronous operations so were used in conjunction with
...
* The signature of `com.hazelcast.jet.pipeline.Sinks#mapWithEntryProcessor`
method has changed, in order to reflect the way in which `EntryProcessor`
has become more lambda-friendly in IMDG (see
link:https://docs.hazelcast.org/docs/latest/manual/html-single/#introducing-lambda-friendly-interfaces[relevant section]
...
* Lambdas have been moved to become the last parameter, where
applicable, for sources and sinks.
