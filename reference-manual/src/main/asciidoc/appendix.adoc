...
https://docs.hazelcast.org/docs/latest/manual/html-single/#introducing-lambda-friendly-interfaces[relevant section]
in the IMDG Migration Guide). The return type of the `EntryProcessor` is
now an explicit parameter in ``mapWithEntryProcessor``'s method signature.
+
[source]
----
...
* We renamed the HDFS source and sink to `Hadoop.inputFormat` and
 `Hadoop.outputFormat`.
...
* We slightly tweaked the generic wildcard parameters in the `Traverser`
type. This change shouldn't affect anything in practice.

* In sources and sinks we changed the method signatures so that the lambda
becomes the last parameter, where applicable.

* We moved `JetBootstrap.getInstance()` to `Jet.bootstrappedInstance()`
...
* We renamed `JobConfig.addResource(...)` to `addClasspathResource(...)`.

* We labeled `ResourceType`, `ResourceConfig` and `JobConfig.getResourceConfigs()`
as private API and discourage their direct usage. We also renamed
`ResourceType.REGULAR_FILE` to `ResourceType.FILE`, but this is now an
internal change.
