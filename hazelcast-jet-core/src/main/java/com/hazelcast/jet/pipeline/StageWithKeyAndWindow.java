...
public interface StageWithKeyAndWindow<T, K> {
     ...
     *
     * <pre>{@code
     *     StreamStage<String> addAggregation(StageWithKeyAndWindow<String> inputStage) {
     *          return inputStage
     *              .aggregate(...)
     *              .mapUsingContext(...); // enrich the result
     *     }
     ...
     */
    @Nonnull
    default <R> R apply(@Nonnull FunctionEx<? super StageWithKeyAndWindow<T, K>, ? extends R> transformFn) {
