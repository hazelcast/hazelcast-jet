...
public final class FileSourceBuilder {
     ...
     * source expects a single {@link JsonObject} or an array of them as the
     * content of the file.
     <p>
     * The source does not save any state to snapshot. If the job is restarted,
     * it will re-emit all entries.
     ...
     *
     */
    public BatchSource<JsonObject> buildJson() {
        String charsetName = charset.name();

        return build(path ->
                new StreamJsonParser(new InputStreamReader(new FileInputStream(path.toFile()), charsetName)).stream());
    }
