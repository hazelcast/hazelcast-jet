...
import static com.hazelcast.jet.core.WatermarkSourceUtil.NO_DEFAULT_TIME;
...
public class WatermarkSourceUtilTest {
    ...
    public void smokeTest() {
        ...
        assertTraverser(wsu.handleEvent(ns(1), null, 0, NO_DEFAULT_TIME));
        // now idle timeout passed for all partitions, IDLE_MESSAGE should be emitted
        assertTraverser(wsu.handleEvent(ns(5), null, 0, NO_DEFAULT_TIME), IDLE_MESSAGE);
        // still all partitions are idle, but IDLE_MESSAGE should not be emitted for the second time
        assertTraverser(wsu.handleEvent(ns(5), null, 0, NO_DEFAULT_TIME));
        // now we observe event on partition0, watermark should be immediately forwarded because the other queue is idle
        assertTraverser(wsu.handleEvent(ns(5), 100L, 0, NO_DEFAULT_TIME), wm(100 - LAG), 100L);
        // now we'll have a event on the other partition. No WM is emitted because it's older than already emitted one
        assertTraverser(wsu.handleEvent(ns(5), 90L, 0, NO_DEFAULT_TIME), 90L);
        assertTraverser(wsu.handleEvent(ns(5), 101L, 0, NO_DEFAULT_TIME), wm(101 - LAG), 101L);
    }
    ...
    public void smokeTest_disabledIdleTimeout() {
        ...
        assertTraverser(wsu.handleEvent(10L, 0, NO_DEFAULT_TIME), 10L);
        assertTraverser(wsu.handleEvent(11L, 0, NO_DEFAULT_TIME), 11L);
        // now have some events in the other partition, wms will be output
        assertTraverser(wsu.handleEvent(10L, 1, NO_DEFAULT_TIME), wm(10 - LAG), 10L);
        assertTraverser(wsu.handleEvent(11L, 1, NO_DEFAULT_TIME), wm(11 - LAG), 11L);
        // now partition1 will get ahead of partition0 -> no WM
        assertTraverser(wsu.handleEvent(12L, 1, NO_DEFAULT_TIME), 12L);
        // another event in partition0, we'll get the wm
        assertTraverser(wsu.handleEvent(13L, 0, NO_DEFAULT_TIME), wm(12 - LAG), 13L);
    }
    ...
    public void test_zeroPartitions() {
        ...
        assertTraverser(wsu.handleEvent(10L, 0, NO_DEFAULT_TIME), wm(10 - LAG), 10L);
    }
    ...
    public void when_idle_event_idle_then_twoIdleMessagesSent() {
        ...
        wsu.increasePartitionCount(1);
        assertTraverser(wsu.handleEvent(ns(0), 10L, 0, NO_DEFAULT_TIME), wm(10 - LAG), 10L);

        // When - become idle
        assertTraverser(wsu.handleEvent(ns(10), null, 0, NO_DEFAULT_TIME), IDLE_MESSAGE);
        // When - another event, but no new WM
        assertTraverser(wsu.handleEvent(ns(10), 10L, 0, NO_DEFAULT_TIME), 10L);
        // When - become idle again
        assertTraverser(wsu.handleEvent(ns(10), null, 0, NO_DEFAULT_TIME));
        assertTraverser(wsu.handleEvent(ns(20), null, 0, NO_DEFAULT_TIME), IDLE_MESSAGE);
    }
    ...
    public void when_eventInOneOfTwoPartitions_then_wmAndIdleMessageForwardedAfterTimeout() {
        ...
        // When
        assertTraverser(wsu.handleEvent(ns(0), 10L, 0, NO_DEFAULT_TIME), 10L);
